...
export const getPrestamos = async (req, res) => {
  try {
    const [rows] = await db.query(`
      ...
      JOIN estadoPrestamo ep ON p.estadoPrestamo = ep.idEstadoPrestamo
...
export const addPrestamo = async (req, res) => {
  ...
  try {
    ...
    const [maxId] = await connection.query(
      "SELECT COALESCE(MAX(idPrestamo), 0) + 1 AS nuevoId FROM prestamo"
    );
    const idPrestamo = maxId[0].nuevoId;

    await connection.query(
      `INSERT INTO prestamo (idPrestamo, nroSocio, ISBN, fechaInicio, 
       fechaDevolucionEsperada, estadoPrestamo) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        idPrestamo,
        nroSocio,
    ...
    await connection.query(
      "UPDATE libro SET estadoLibro = ? WHERE ISBN = ?",
...
export const devolverLibro = async (req, res) => {
  ...
  try {
    ...
    await connection.query(
      `UPDATE prestamo 
       SET fechaDevolucionReal = ?, estadoPrestamo = ? 
       WHERE idPrestamo = ?`,
